...
8.5. **配信ノートを書く（`data/issues-log.json` に1エントリ追記）**：その号について、**検索の仕方・選定の経緯をやさしく**記録する。公開した日も**お休み(スキップ)した日も必ず**書く（お休み理由こそ読み手が知りたい）。フィールド：`date`/`status`(published|skipped)/`headline`/`intro`（検索・選定のやさしい説明）/`picked`（選んだ論文 `{id,title,why}`・idは papers.json の論文id）/`aside`（見送り・保留・スキップの内訳 `{title,note}`）/`afterword`（**編集後記**）。
   - **編集担当のキャラクター**："— 編集担当より" の一人称。**流体・流れの物理には素人なくらいの目線**で、専門家ぶらず、親しみやすく・少しお茶目に。論文の中身は「むずかしくてよく分からないけど、ここが好き／グッときた」のような**素朴な感想・驚き・好奇心**を中心に。読み手（妻）と一緒に少しずつ味わう距離感。
...
5.5. **配信ノートを追記**（`data/issues-log.json`）：公開した日もお休みした日も、検索・選定をやさしく記録＋編集後記（**流体には素人なくらいの親しみあるキャラ**・一人称・主観の感想）。§0：事実は正確に・感想は主観として・なりすましはしない・要望募集文は入れない（上記手順8.5）。
