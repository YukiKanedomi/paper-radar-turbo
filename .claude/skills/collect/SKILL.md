...
2. **選定**（流体主役）：定番(classic)＋最新(latest)の2件。**最低1件はOA全文**。
   - **classic も OA を優先（C）**：候補に on-topic な **OA定番**があればそれを優先（両方OAでよい＝§0が楽で読み応えも出る）。非OA(抄録のみ)の定番は、適切なOA定番が無い時の最終手段（抄録ベースは§0が厳しく保留になりやすい）。`collect.mjs` 出力の「OA全文 N件」「⚠OA定番なし」表示で判断。
...
8.5. **配信ノートを書く（`data/issues-log.json` に1エントリ追記）**：その号について、**検索の仕方・選定の経緯をやさしく**記録する。公開した日も**お休み(スキップ)した日も必ず**書く（お休み理由こそ読み手が知りたい）。フィールド：`date`/`status`(published|skipped)/`headline`/`intro`（検索・選定のやさしい説明）/`picked`（選んだ論文 `{id,title,why}`・idは papers.json の論文id）/`aside`（見送り・保留・スキップの内訳 `{title,note}`）/`afterword`（**編集後記＝人間味のある一個人の感想**）/`wish`（次への一言）。
   - **§0と両立**：`afterword` は温かい一人称（"— 編集担当より"）の**主観的な感想**でよいが、**事実の捏造はしない**（年・被引用・主張など事実部分は正確に）。**架空の人物になりすまさない**（実在の特定個人を装わない）。感想は感想、事実は事実とわかる書き方に。`intro`/`aside` も実際に起きたこと（504・保留・スキップ理由）をやさしく正直に。
...
1. 候補取得（上記1）→ **自動選定**：流体主役・定番＋最新・**必ず1件OA全文**。**classic も OA を優先（C）**＝on-topic な OA定番があれば非OAより優先（両方OA可）。実図を出すなら CC-BY/CC0/PD を優先。ML/手法は主役にしない。`collect.mjs` 出力の「⚠最新0件 / ⚠OA定番なし」警告も判断材料に。
...
4. **忠実性ゲート（必須）**：各論文の要約を `faithfulness-check` サブエージェントに渡す。
   ...
   - **フォールバック（D）**：保留が出たら**同ストリームの次点候補**（`collect.mjs` の候補プール上位から on-topic な順・classicはOA優先）に切り替えて作り込み→忠実性ゲートに掛ける。**数件試して**§0を通る2件が揃うか確認してから公開判定へ（即スキップしない）。
5. **公開判定**：
...
5.5. **配信ノートを追記**（`data/issues-log.json`）：公開した日もお休みした日も、検索・選定をやさしく記録＋編集後記（人間味のある一個人の感想）。§0：事実は正確に・感想は主観として・なりすましはしない（上記手順8.5）。
