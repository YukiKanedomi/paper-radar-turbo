...
export const CATEGORIES: GraphCategory[] = [
  {
    ...
    terms: [
      ...
      "換気キャビテーション (ventilated cavitation)",
      "換気係数 (ventilation coefficient, C_q)",
      "パフィング (puffing)",
      "ディボット (divot)",
  ...
  },
  {
    ...
    terms: [
      ...
      "混合損失",
      "端壁（二次流れ）損失",
      "二次流れ誘起損失 (secondary-flow-induced loss)",
      "境界層散逸損失 (boundary-layer dissipation loss)",
      "流れ方向渦度 (streamwise vorticity)",
      "翼端漏れ",
      "境界層",
      "翼端漏れ渦",
  ...
  },
  {
    ...
    terms: [
      ...
      "リーディングエッジタバクル (leading-edge tubercle)",
...
export const CURATED_EDGES: CuratedEdge[] = [
  ...
  { from: "端壁（二次流れ）損失", to: "二次流れ誘起損失 (secondary-flow-induced loss)", label: "内訳の一つ" },
  { from: "端壁（二次流れ）損失", to: "境界層散逸損失 (boundary-layer dissipation loss)", label: "内訳の一つ" },
  { from: "二次流れ誘起損失 (secondary-flow-induced loss)", to: "流れ方向渦度 (streamwise vorticity)", label: "この指標でスケール" },
  { from: "換気キャビテーション (ventilated cavitation)", to: "キャビテーション数 (cavitation number)", label: "形成条件を特徴づける" },
  { from: "ハブ渦 (hub vortex)", to: "後流（ウェイク）", label: "近傍後流を構成" },
  { from: "ウィンドファーム誘導域 (wind-farm induction zone)", to: "後流（ウェイク）", label: "後流重畳で発生" },
  { from: "リーディングエッジタバクル (leading-edge tubercle)", to: "前縁剥離 (leading-edge separation)", label: "抑制" },
  { from: "後退翼の逆流 (reverse flow on retreating blade)", to: "前縁剥離 (leading-edge separation)", label: "誘発" },
  { from: "ピッチングモーメント衝撃 (pitching moment impulse)", to: "後退翼の逆流 (reverse flow on retreating blade)", label: "帰結" },
  { from: "換気係数 (ventilation coefficient, C_q)", to: "換気キャビテーション (ventilated cavitation)", label: "注入量を無次元化" },
  { from: "パフィング (puffing)", to: "ディボット (divot)", label: "注入位置で入れ替わる界面擾乱" },
  ...
  { from: "マイルドサージ (mild surge)", to: "サージ", label: "逆流を伴わない軽い形態" },
  { from: "マイルドサージ (mild surge)", to: "深いサージ (deep surge)", label: "さらに絞ると移行" },
  { from: "サージ", to: "リサージュ曲線 (Lissajous curve)", label: "運転点の軌跡を可視化" },
  { from: "翼端漏れ渦", to: "マイルドサージ (mild surge)", label: "スパイラル型崩壊が引き金" },
  { from: "ベーンレスディフューザ (vaneless diffuser)", to: "回転失速", label: "発生箇所" },
  { from: "ベーンレスディフューザ (vaneless diffuser)", to: "ベーン付きディフューザ (vaned diffuser)", label: "対義（羽根の有無）" },
  { from: "分離渦シミュレーション (DES)", to: "遅延分離渦シミュレーション (DDES)", label: "関連手法" },
  { from: "半径方向すき間 (radial gap)", to: "ロータ・ステータ干渉", label: "狭いほど強まる" },
